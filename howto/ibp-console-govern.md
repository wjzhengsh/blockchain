...
| **Orderer storage** | When you anticipate that this orderer will be part of an ordering service on many channels. Recall that orderers keep a copy of the blockchain for every channel they're a part of. As the orderer default is 200G, this means as many as 20 million total transactions will fit in storage before it will need to be expanded. While 200G might seem like much more storage than is needed, keep in mind that storage is relatively inexpensive, and that the process for increasing it is more difficult (requires command line) than increasing CPU or memory. |
