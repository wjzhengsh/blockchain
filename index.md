...
{{site.data.keyword.blockchainfull_notm}} Platform is the only integrated business-ready platform that addresses the full lifecycle (**develop**, **govern**, and **operate**) of a multi-organization blockchain network. It is designed to accelerate, through collaboration in each phase, the creation of "built for business" global blockchain networks with the performance and security for even the most demanding use cases and regulated industries. {{site.data.keyword.blockchainfull_notm}} offerings are built on the Hyperledger Fabric code base leveraging a modular architecture to achieve enterprise levels of security, data integrity, scalability, and performance to meet your business needs.  
...
As a developer, you can quickly and easily align business requirements and accelerate blockchain application development in  {{site.data.keyword.blockchainfull_notm}} Platform network environment, using an interactive playground to develop, iterate and test business networks. These tools are designed to turn business rules into business network code in your preferred environment:
* **Explore online**  
  Leverage [{{site.data.keyword.blockchainfull_notm}} Platform: Develop](./develop.html), which is owered by an open source development tool to learn key blockchain concepts, create network definitions, and leverage reusable industry models and smart contract libraries.  
...
The **modularity** of the Hyperledger Fabric architecture and the distinct separation of network roles (endorsor, committer, orderer) provide an infrastructure enabling scalability and adaptability to a wide variety of use cases.
...
For complete details on all Hyperledger Fabric features and functionality,
