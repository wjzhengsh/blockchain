...
**Foundational Network Services** - To make a blockchain operational, the members form a foundation of trust by running one or more foundational network services: 

* Peer Service -  Validation/Endorsing transactions
* Certificate Authority (CA) - Issuing Certificates to participants 
* Ordering Service - Ordering and synchronizing transactions
 
When a member joins a network via the {{site.data.keyword.blockchainfull_notm}} Platform, the standard issuance is a Peer and a CA.  For a production network, members will want to run multiple instances of these services to ensure availability.
 
By default, IBM runs the Ordering Service for networks created by the {{site.data.keyword.blockchainfull_notm}} Platform.
