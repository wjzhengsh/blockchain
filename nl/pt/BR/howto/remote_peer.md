...
- Peers do [Gossip](../glossary.html#gossip) for AWS não são suportados atualmente. Isso significa que os recursos do Fabric que dependem do gossip, como [dados privados ![Ícone de link externo](../images/external_link.svg "Ícone de link externo")](https://hyperledger-fabric.readthedocs.io/en/release-1.2/private-data-arch.html "dados privados") e [descoberta de serviço ![](../images/external_link.svg  "Ícone de link externo")](https://hyperledger-fabric.readthedocs.io/en/release-1.2/discovery-overview.html "descoberta de serviço") também não são suportados.
...
**Nota:** para operar um peer AWS, deve-se ter uma organização que pertença a uma rede Starter Plan ou Enterprise Plan no {{site.data.keyword.blockchainfull_notm}} Platform. Isso implica que você ou outro membro na rede deve pagar a {{site.data.keyword.blockchainfull_notm}} [taxa de associação](pricing.html#key-elements-of-pricing) para sua organização. Para obter mais informações sobre o pagamento de taxas, consulte [Modo de pagamento](paying_mode.html).
...
Para obter instruções sobre como implementar o {{site.data.keyword.blockchainfull_notm}} Platform for AWS, consulte [Implementando peers no Amazon Web Services](remote_peer_aws.html "Implementando peers no Amazon Web Services").
