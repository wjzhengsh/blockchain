...
Depois de registrar a identidade do peer, também é necessário criar uma identidade do administrador que você usará para operar o peer. Primeiro, é necessário registrar essa nova identidade com sua CA e usá-la para gerar uma pasta MSP. Em seguida, você incluirá os usuários administrativos signCert no arquivo de configuração, no qual será criado um certificado de administrador do peer durante a implementação. Isso permite que você use os certificados da identidade do administrador para operar sua rede de blockchain, como iniciando um novo canal ou instalando o chaincode em seus peers.

Você precisa apenas criar uma identidade do administrador para os componentes pertencentes à sua organização. Se você estiver implementando vários peers, será necessário concluir essas etapas apenas uma vez. É possível usar o signCert gerado para um peer para implementar quaisquer outros peers pertencentes à sua organização.
...
Depois de registrar a identidade do administrador é necessário gerar a pasta MSP do administrador do peer e o signCert. Portanto, é necessário executar um comando de inscrição com relação à sua CA do Starter Plan ou do Enterprise Plan.
...
6. Emita o comando a seguir para gerar certificados com sua identidade do administrador de peer:
  ...
  O `<enroll_id>` e o `<enroll_password>` acima são o **ID** e o **Segredo** do administrador de peer que foi [registrado usando o Network Monitor](/docs/services/blockchain/howto/peer_deploy_ibp.html#ibp-peer-deploy-register-admin). O `<ca_name>` e o `<ca_url_with_port>` são os valores do `caName` e da `url` de seu perfil de conexão. Deixe `http://` fora do início da URL de CA.
...
3. Execute o comando a seguir para gerar certificados com o administrador de CA TLS.
  ...
  No comando, o `<enroll_id>` e a `<enroll_password>` são o [nome de usuário e a senha do administrador da CA](/docs/services/blockchain/howto/CA_deploy.html#ca-deploy-admin-secret) transmitidos ao segredo do Kubernetes quando você implementou a Autoridade de Certificação. Insira a [URL da CA](/docs/services/blockchain/howto/CA_operate.html#ca-operate-url) dentro do `<ca_url_with_port>`. Deixe fora o `http://` no início. O `<tls_ca_name>` foi especificado por você durante a [configuração da CA](/docs/services/blockchain/howto/CA_deploy_icp.html#ca-deploy-configuration-parms).

  O `<ca_tls_cert_file>` é o nome do arquivo de seu [certificado TLS da CA](/docs/services/blockchain/howto/CA_operate.html#ca-operate-tls) com o caminho completo.
...
| `Peer Docker image tag`|Valor da tag associada à imagem do peer |1.4.0, preenchida automaticamente para o valor correto.|sim|
...
| `Docker-in-Docker CPU request`| Especifique o número mínimo de CPUs a serem alocadas para o contêiner no qual o chainode é executado. | 1 | sim |
