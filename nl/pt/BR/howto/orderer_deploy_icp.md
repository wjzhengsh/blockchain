...
É necessário determinar o armazenamento que seu solicitador usará. Se você usar as configurações padrão, o gráfico Helm criará uma nova Solicitação de Volume Persistente (PVC) de 8 Gi com o nome de `meu-data-pvc` para os dados de seu solicitador e outros 8 Gi de PVC com o nome `statedb-pvc` para seu banco de dados de estado.
...
Se você não usar o fornecimento dinâmico, os [Volumes Persistentes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) deverão ser criados e configurados com rótulos que possam ser usados para refinar o processo de ligação PVC do Kubernetes.
...
Depois de salvar o arquivo de configuração, é necessário codificá-lo no formato base64 antes de fornecer o arquivo para o ICP como um objeto secreto. Um segredo do Kubernetes permite que você proteja e compartilhe informações sem ter que passá-las diretamente para a implementação.
...
**Nota:** o segredo da configuração do solicitador não será removido de seu cluster de ICP quando você excluir sua liberação do Helm. Se você excluir seu solicitador, será necessário excluir esse segredo também.

## Configuração
{: #orderer-configuration}

Depois de criar seu objeto secreto de configuração do solicitador, é possível configurar e instalar seu solicitador no ICP com as etapas a seguir. É possível instalar apenas um solicitador por vez.
...
4. Especifique os valores para os [Parâmetros de configuração geral](#orderer-global-parameters) e aceite o contrato de licença.
5. Abra a seta `Todos os parâmetros` e especifique o valor para os [Parâmetros de configuração global](#orderer-global-parameters).
6. Role para baixo até a seção **Configuração do solicitador**. Marque a caixa de seleção `Instalar solicitador` e conclua as [definições de configuração](#orderer-parameters) para o solicitador.
...
| `Orderer worker node architecture`| Selecione a arquitetura de nó do trabalhador do ICP (AMD64 ou S390X). | Arquitetura de detecção automática com base no nó principal | sim |
...
| `Orderer data persistence enabled` | Os dados estarão disponíveis quando o contêiner for reiniciado. Se desmarcado, todos os dados serão perdidos no caso de um failover ou de uma reinicialização de pod. | verificado | não |
| `Orderer use dynamic provisioning` | Marque para ativar o fornecimento dinâmico para volumes de armazenamento. | verificado | não |
...
| `Orderer storage class name`| Especifique um nome de classe de armazenamento para o solicitador. | nenhum | Depende de como o cluster do ICP está configurado. Verifique com seu administrador de cluster |
...
Os logs de componentes podem ser visualizados por meio da linha de comandos, usando os [`kubectl CLI commands`](orderer_operate.html#orderer-kubectl-configure) ou por meio do [Kibana ![Ícone de link externo](../images/external_link.svg "Ícone de link externo")](https://www.elastic.co/products/kibana "Sua janela no Elastic Search"), que está incluído em seu cluster do ICP. Para obter mais informações, consulte estas [instruções para acessar os logs](orderer_operate.html#orderer-view-logs).
