...
Depois de instalar o solicitador do {{site.data.keyword.blockchainfull}} Platform no ICP, um configmap é criado e ele contém configurações padrão para variáveis de ambiente. Em seguida, é possível mudar ou incluir variáveis de ambiente para o solicitador para configurar seu comportamento.
...
É necessário concluir algumas etapas de pré-requisito de seu cluster de ICP para operar seu solicitador.

**Pré-requisitos:**
- [Configurar suas CLIs](#orderer-kubectl-configure)
- [Recuperar suas informações sobre terminais do solicitador](#orderer-endpoint)
- [Fazer download de seu certificado TLS do solicitador](#orderer-tls)
...
É necessário usar a ferramenta de linha de comandos **kubectl** para se conectar ao contêiner do solicitador que é executado no ICP.
...
5. Se você ainda não o tiver feito, siga as instruções para configurar a [CLI kubeclt](#orderer-kubectl-configure). É necessário usá-lo para interagir com seu contêiner do solicitador.
...
3. Localize o nome de sua Liberação do Helm e abra o painel de detalhes da Liberação do Helm.
4. Role para baixo até a seção **Notas** na parte inferior do painel. A seção **Notas** inclui um conjunto de comandos para ajudar a operar sua implementação do solicitador.
5. Se você ainda não o tiver feito, siga as instruções para configurar a [CLI kubeclt](#ca-kubectl-configure). É necessário usá-lo para interagir com seu contêiner do solicitador.
...
2. Assegure-se de [fazer download do certificado TLS de seu solicitador](#orderer-tls) e de poder referenciá-lo em sua linha de comandos. Se você seguiu os comandos de exemplo nesta documentação, será possível localizar esse certificado TLS no arquivo `$HOME/fabric-ca-client/orderer-tls/orderertls.pem`.
...
Quando você implementa um solicitador usando o gráfico Helm do {{site.data.keyword.blockchainfull_notm}} Platform for ICP, o bloco genesis é criado automaticamente com a organização do solicitador como o único administrador do canal do sistema. Isso significa que o administrador da organização do solicitador possui a capacidade exclusiva de incluir organizações no consórcio. A política que controla isso, que é conhecida como `mod_policy` para o canal do sistema do solicitador, não deve ser mudada depois que novas organizações são incluídas no canal do sistema do solicitador.
...
Observe que é possível incluir uma organização em um canal sem primeiro associar-se ao canal do sistema. Para obter mais informações, consulte [Incluindo uma organização em um tutorial de canal](https://hyperledger-fabric.readthedocs.io/en/release-1.2/channel_update_tutorial.html) na documentação do Hyperledger Fabric.
...
- [peer](https://hyperledger-fabric.readthedocs.io/en/release-1.2/commands/peercommand.html), que permitirá que você busque o bloco genesis e atualize o canal do sistema.
- [configtxlator](https://hyperledger-fabric.readthedocs.io/en/release-1.2/commands/configtxlator.html), que converte o formato protobuf de uma configuração de canal para o formato JSON que é mais fácil de ler e atualizar.
...
1. Configure as variáveis de ambiente a seguir usando seu nome do MSP da organização do solicitador que foi estabelecido durante a implementação, o caminho para seu MSP do solicitador e o caminho para o certificado TLSCA de seu solicitador.
  ...
  Substitua os campos por suas próprias informações.
    ...
    - Substitua `<PROXY_IP>` pelo endereço IP do proxy das [informações do terminal do solicitador](#orderer-endpoint)
    - Substitua `<EXTERNAL_NODE_PORT>` pela porta do nó externo das [informações do terminal do solicitador](#orderer-endpoint)
...
3. Busque o bloco genesis do canal do sistema. Primeiro, crie um diretório chamado `configupdate` dentro de `org-definitions` para armazenar os arquivos de configuração gerados durante o processo de atualização de configuração. É possível chamar esse diretório do que desejar, mas os comandos de amostra a seguir usarão `configupdate.`
  ...
  Substitua `<orderer_TLS_root_cert_file>` pelo caminho para o arquivo `orderertls.pem` que você criou nesta [etapa](#orderer-tls). Por exemplo, seu comando pode ser semelhante ao exemplo a seguir:
