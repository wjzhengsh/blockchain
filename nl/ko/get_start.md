...
5. 체인 코드를 설치하고 인스턴스화하십시오. 모든 채널 구성원은 체인코드를 실행할 모든 피어에 이름과 버전이 같은 동일한 체인코드를 설치해야 합니다. 체인코드를 설치한 후, 이 체인코드를 사용하려면 먼저 채널에서 인스턴스화해야 합니다. 자세한 정보는 [체인코드 설치 및 인스턴스화](howto/install_instantiate_chaincode.html)를 참조하십시오.  
