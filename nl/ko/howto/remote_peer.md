...
**참고** {{site.data.keyword.blockchainfull}} Platform Remote Peer on ICP(베타) 프로그램은 종료되었습니다. 계속 ICP 환경에서 피어를 실행하려면 **{{site.data.keyword.blockchainfull_notm}} Platform for ICP** 오퍼링을 대신 사용하십시오. 자세한 정보는 [ICP의 피어 정보](../ibp-for-icp-about.html#ibp-icp-peer)를 참조하십시오.
...
- Fabric v1.1 또는 1.2.1 레벨에 있는 블록체인 네트워크에만 {{site.data.keyword.blockchainfull_notm}} Platform for AWS 피어를 연결할 수 있습니다. 네트워크 모니터에서 [네트워크 환경 설정 창](../v10_dashboard.html#network-preferences)을 열어 Fabric 버전을 찾을 수 있습니다.
...
**참고:** AWS 피어를 운영하려면 {{site.data.keyword.blockchainfull_notm}} Platform에서 스타터 플랜 또는 엔터프라이즈 플랜 네트워크에 속한 조직이 있어야 합니다. 즉, 사용자나 네트워크의 다른 구성원이 조직을 위해 {{site.data.keyword.blockchainfull_notm}} [멤버십 요금](pricing.html#key-elements-of-pricing)을 지불해야 합니다. 요금 지불에 관한 자세한 정보는 [지급 모드](paying_mode.html)를 참조하십시오.
...
{{site.data.keyword.blockchainfull_notm}} Platform for AWS를 배치하는 방법에 대한 지시사항은 [Amazon Web Services에서 피어 배치](remote_peer_aws.html "Amazon Web Services에서 피어 배치")를 참조하십시오.
...
AWS 피어를 배치한 다음 여러 운영 단계를 완료해야 피어에서 네트워크에 트랜잭션을 제출할 수 있습니다. 이 운영 단계에는 채널에 조직을 추가하고 채널에 피어를 가입하며 피어에 체인코드를 설치하고 채널에서 체인코드를 인스턴스화하며 애플리케이션을 피어에 연결하는 단계가 포함됩니다. 자세한 정보는 [Amazon Web Service에서 피어 운영](remote_peer_operate_aws.html#remote-peer-operate-aws)을 참조하십시오.
...
IBM Blockchain Platform에서는 이 오퍼링을 지원하지 않습니다. 피어와 관련된 문제가 발생하면 무료 블록체인 개발자 리소스 및 지원 포럼을 사용하여 {{site.data.keyword.IBM_notm}}과 Fabric 커뮤니티에서 도움을 받을 수 있습니다. 자세한 정보는 [블록체인 리소스 및 지원 포럼](../ibmblockchain_support.html#resources)의 내용을 참조하십시오. 네트워크 모니터의 **도움말** 화면에서 지원 리소스도 볼 수 있습니다.
