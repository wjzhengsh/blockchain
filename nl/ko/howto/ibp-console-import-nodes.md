...
피어를 콘솔 내의 채널에 가입시키려고 할 때 피어 가져오기 옵션을 사용할 수 있습니다. {{site.data.keyword.blockchainfull_notm}} Platform 2.0 서비스는 다중 {{site.data.keyword.cloud_notm}} 환경에 걸쳐 배치되므로 일부 서비스 인스턴스에는 CA 및 피어만 포함되고 다른 서비스 인스턴스는 순서 지정 서비스를 호스팅할 가능성이 높습니다. 콘솔에
...
조직에 더 많은 피어를 추가하는 데 이를 사용할 수 있습니다. 또는 CA를 사용하여 피어 또는 순서 지정 서비스에 대해 추가 관리자 ID를 작성할 수 있습니다.
...
- 순서 지정 서비스의 관리자 ID JSON 파일을 콘솔 지갑으로 가져왔는지 확인하십시오.
- 작성된 콘솔에서 내보낸 순서 지정 서비스 JSON 파일이 사용 가능한지 확인하십시오.

### 순서 지정 서비스를 가져오는 방법
순서 지정 서비스 가져오기는 **노드** 탭에서 수행됩니다. 순서 지정 서비스 섹션에서 **순서 지정 서비스 추가** `+` 부호를 클릭하십시오. 그 다음으로 **기존 순서 지정 서비스 가져오기**를 클릭하십시오. 표시되는 사이드 패널의 단계에 따라
수동으로 연결 정보를 지정하십시오.
- **JSON 업로드** 단추를 클릭하여 정보의 수동 입력을 건너뛰고 순서 지정 서비스가 작성된 콘솔에서 내보낸 JSON 파일을 찾아보십시오.
- **기존 ID**를 클릭하여 순서 지정 서비스의 관리자 ID를 설정하고 콘솔 지갑에서 순서 지정 서비스 관리자 ID를 선택하십시오.
