...
다른 ID로 전환하여 CA 관리자로 사용하려면 **설정** 아이콘을 클릭한 후 슬라이더에서 **ID 설정**을 클릭하십시오. **기존 ID** 탭을 사용하여 지갑에 존재하는 ID를 지정할 수 있습니다. 또는 **새 ID** 탭에서 새 관리자에 대한 인증서(base64 또는 PEM 형식)가 포함된 파일을 업로드하거나 인증서가 포함된 JSON 파일을 업로드할 수 있습니다.
...
- **인증서:** CA를 사용하여 생성한 ID의 공개 키(base64 또는 PEM 형식)가 포함된 파일을 업로드하십시오. 
