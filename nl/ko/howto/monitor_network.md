...
- **피어 로그**  
  피어 로그에는 체인코드를 설치, 인스턴스화 및 호출한 결과가 포함되어 있습니다. 체인코드 이름과 버전을 검색하여 특정 체인코드의 로그를 찾을 수 있습니다. [채널 모니터의 체인코드 섹션](#monitor-channel-cc)에서 특정 체인코드의 로그도 볼 수 있습니다. 트랜잭션 제안에서 생성하는 메시지 또는 제안 요청과 관련된 제한시간 초과 문제는 피어 로그에서 찾을 수 있습니다. 피어 로그에는 [체인코드의 보증 정책](install_instantiate_chaincode.html#endorsement-policy)을 충족시키지 않아서 거절된 트랜잭션의 오류도 포함되어 있습니다. 채널 가입 요청의 결과를 찾을 수도 있습니다.
