...
{{site.data.keyword.cloud_notm}} Private(ICP) 피어에 {{site.data.keyword.blockchainfull}} Platform을 설정한 후 여러 운영 단계를 완료해야 피어에서 스타터 플랜 또는 엔터프라이즈 플랜 네트워크에 트랜잭션을 제출할 수 있습니다. 이 단계에는 채널에 조직을 추가하고 채널에 피어를 가입하며 피어에 체인코드를 설치하고 채널에서 체인코드를 인스턴스화하며 애플리케이션을 피어에 연결하는 단계가 포함됩니다.
{:shortdesc}
...
- [사용자의 피어 엔드포인트 정보 검색](#peer-endpoint)
- [사용자의 피어 TLS 인증서 다운로드](#peer-tls)

다음 방법 중 하나를 사용하여 피어를 작동시킬 수 있습니다.
...
SDK를 사용할지 아니면 명령행을 사용할 계획인지에 상관없이 피어 오퍼레이션 과정에서 이 섹션의 단계를 완료해야 합니다. 시작하기 전에 이러한 모든 단계를 완료할 수 있습니다.

### CLI 구성
{: #peer-kubectl-configure}

ICP에서 실행되는 피어 컨테이너에 연결하려면 **kubectl** 명령행 도구를 사용해야 합니다.
...
1. ICP 클러스터 UI에 로그인하십시오. **명령행 도구** 탭으로 이동하여 **Cloud Private CLI**를 클릭하십시오. 다운로드할 수 있는 다음 도구가 표시됩니다.
  ...
  피어를 운영하려면 첫 번째 **세 가지** 도구를 사용해야 하며 Helm은 선택사항입니다. 각각을 클릭하고 사용하고 있는 머신 유형에 해당하는 `curl` 명령을 실행해야 합니다. 그런 다음 각 도구에 대해 `chmod` 및 `sudo mv` 명령을 실행하십시오. `chmod` 명령은 해당 CLI의 권한을 실행 가능하게 변경하고 `sudo mv` 명령은 파일을 이동하고 이름을 바꿉니다.

  첫 번째 도구 **cloudctl**의 명령은 다음 예제와 같습니다.
  ...
  두 번째 도구 **kubectl**의 명령은 다음 예제와 같습니다.
...
2. **kubectl**을 구성한 후 다음 명령을 실행하십시오.
  ...
  성공하면 명령은 다음 예와 유사한 응답을 리턴합니다. 여기서 `peer-6cf85f6687-hcxpl`은 피어 컨테이너의 이름입니다.

  ```
  이름                           준비          상태         재시작      유효기간
  peer-6cf85f6687-hcxpl          2/2        실행 중            0          6시간
  ```

  이제 피어 엔드포인트 정보를 가져올 **kubectl** 도구를 사용할 준비가 되었습니다.

3. 선택적으로 **helm**을 사용하려면 몇 가지 추가 단계를 완료하십시오. Helm은 설치 시 선택 사항이며 이 지시사항에서 사용할 필요가 없음을 유의하십시오. 그러나 Helm 릴리스를 관리하고 ICP에 배치할 사용자 소유의 아카이브를 작성하는 것은 유용할 수 있습니다.
  ...
  3. `darwin-amd64`에 `/helm`을 추가하여 `sudo mv` 명령을 실행하십시오. Helm에 대해 `chmod` 명령을 실행할 필요는 없습니다. 예를 들어, 다음과 같습니다.
  ...
  Helm이 정상적으로 설치되었는지 확인하려면 `helm help` 명령을 실행할 수 있습니다.

### 피어 엔드포인트 정보 검색
{: #peer-endpoint}

채널에 가입하거나 스마트 계약을 설치하려면 SDK 또는 Fabric CA Client에서 피어 엔드포인트를 대상으로 지정해야 합니다. ICP 콘솔 UI를 사용하여 피어 엔드포인트를 찾을 수 있습니다. 다음 단계를 완료하려면 [클러스터 관리자 ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.1.0/user_management/assign_role.html "클러스터 관리자 역할 및 조치")여야 합니다.

1. ICP 콘솔에 로그인하고 왼쪽 상단에서 **메뉴** 아이콘을 클릭하십시오.
2. **워크로드** > **Helm 릴리스**를 클릭하십시오.
3. Helm 릴리스 이름을 찾아 Helm 릴리스 세부사항 패널을 여십시오.
4. 아래로 스크롤하여 패널의 맨 아래에 **참고** 섹션으로 이동하십시오. **참고** 섹션에는 피어 배치를 작동시키는 데 도움이 되는 명령 세트가 표시됩니다.
5. 아직 구성되지 않은 경우 [kubeclt CLI](#peer-kubectl-configure)를 구성하는 단계를 따르십시오. kubectl을 사용하여 피어 컨테이너와 상호작용할 수 있습니다.
6. CLI에서 **1. 다음 명령을 실행하여 애플리케이션 URL 가져오기:** 다음에 오는 참고의 첫 번째 명령을 실행하십시오. 이 명령은 피어 URL과 포트를 출력하며 다음 예와 유사합니다. 이후 명령에 필요한 이 URL을 저장하십시오.
...
**참고:** 방화벽 뒤에 피어를 배치할 경우 패스스루를 열어 플랫폼의 네트워크에서 피어와 TLS 핸드쉐이크를 완료할 수 있어야 합니다. 피어의 포트 7051에 바인딩된 노드 포트에 대해서만 패스스루를 사용으로 설정하면 됩니다. 자세한 정보는 [피어 엔드포인트 정보 찾기](#peer-endpoint)를 참조하십시오.

### 피어 TLS 인증서 다운로드
{: #peer-tls}

원격 클라이언트에서 피어와 통신하려면 피어 TLS 인증서를 다운로드하여 명령에 전달해야합니다.
...
4. 아래로 스크롤하여 패널의 맨 아래에 **참고** 섹션으로 이동하십시오. **참고** 섹션에는 피어 배치를 작동시키는 데 도움이 되는 명령 세트가 표시됩니다.
5. 아직 구성되지 않은 경우 [kubeclt CLI](#peer-kubectl-configure)를 구성하는 단계를 따르십시오. kubectl을 사용하여 피어 컨테이너와 상호작용할 수 있습니다.
6. CLI에서 **3. 피어 TLS 루트 인증서 파일 가져오기** 다음에 나오는 참고의 첫 번째 명령을 실행하십시오. 이 명령은 TLS 인증서를 로컬 시스템에 파일 `cacert.pem`으로 저장합니다.
7. 이후 명령에서 인증서를 참조할 수 있는 위치로 인증서를 옮기고 이름을 `peertls.pem`으로 변경하십시오.
...
SDK와의 통신을 인증하려면 피어의 TLS 인증서를 참조해야 합니다. [피어 컨테이너에서 다운로드](#peer-tls)한 TLS 인증서를 찾아 애플리케이션에서 참조할 수 있는 위치에 저장하십시오. 예제 명령을 사용한 경우 `$HOME/fabric-ca-client/peer-tls/peertls.pem`에서 피어 TLS 인증서를 찾을 수 있습니다. 그런 다음 간단한 읽기 파일 명령을 사용하여 TLS 인증서를 애플리케이션에 가져올 수 있습니다.
...
[피어의 엔드포인트 정보](#peer-endpoint)를 찾아 새 피어 변수를 선언하거나 연결 프로파일을 업데이트하여 SDK에 제공하십시오. 다음 예에서는 패브릭 네트워크에서 피어를 엔드포인트로 정의하고 가져온 TLS 인증서를 전달합니다.
...
**참고:** 피어가 {{site.data.keyword.cloud_notm}, other organizations on the {{site.data.keyword.blockchainfull_notm}} Platform 네트워크 외부에 있으므로 연결 프로파일에서 피어의 엔드포인트 정보를 찾을 수 없습니다. 다른 조직에서 인증할 트랜잭션을 피어에 보내면 대역 내외 오퍼레이션에서 피어 url을 제공해야 합니다.
...
이 명령은 바이너리를 `bin/` 디렉토리에 설치합니다. 위의 Fabric 도구를 다운로드한 경로를 도구에 설정하십시오.
```
export PATH=$PATH:<full_path_to_bin_folder>
```
{:codeblock}

예를 들어, 홈 디렉토리에 바이너리를 설치한 경우 `PATH`를 다음과 같이 설정합니다.
...
피어 관리자 MSP 폴더가 생성된 디렉토리를 전환하십시오. 이 문서의 예제 단계를 수행했다면 아래와 유사한 디렉토리에 MSP 폴더가 있습니다.
...
2. [피어의 엔드포인트 정보 찾기](#peer-endpoint). `PEERADDR` 환경 변수를 설정하려면 피어 엔드포인트를 사용해야 합니다. 시작 부분에 `http://`를 제외해야 합니다.
...
3. 다음 명령을 실행하여 환경 변수를 설정하십시오.
  ...
  고유 정보로 필드를 바꾸십시오.
    - `<full_path_to_config_folder>`를 [피어 클라이언트를 다운로드](#peer-client)를 할 때 작성한 구성 폴더로 바꾸십시오.
  ...
  다음 명령을 실행하여 수행할 수 있는 TLS를 사용으로 설정하고자 할 수도 있습니다.
...
이제 피어에 체인코드를 설치하고 인스턴스화할 준비가 되었습니다. 이 지시사항에서 사용하기 위해 `fabric-samples` 저장소에서 `fabcar`를 설치합니다. 이전에 [GOPATH를 구성 ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://hyperledger-fabric.readthedocs.io/en/release-1.2/dev-setup/devenv.html?highlight=gopath#set-your-gopath "GOPATH 설정")했는지 확인한 후 다음 명령을 사용하여 github에서 `fabric-samples` 체인코드를 다운로드하십시오.
...
**참고:** 이 체인코드가 IBP 스타터 또는 엔터프라이즈 플랜에서 실행 중인 다른 피어에 이미 설치되어 인스턴스화된 경우 피어에 체인코드를 설치하기 전에 수행해야 하는 추가 단계가 있습니다. 이 체인코드 설치 방법과 연관된 오류를 방지하는 방법에 대한 자세한 정보는 [문제점 해결 주제](#peer-ibp-troubleshooting)의 내용을 참조하십시오.

### CLI를 사용하여 채널에서 체인코드 인스턴스화
{: #icp-toolcontainer-instantiate-cc}

한 피어만 채널에서 체인코드를 인스턴스화하면 되므로 피어를 사용하여 체인코드를 인스턴스화하지 않아도 됩니다. {{site.data.keyword.blockchainfull_notm}} Platform에서 호스팅되는 피어에서 해당 작업을 수행할 수 있습니다. 그러나 피어가 채널에서 체인코드를 인스턴스화하게 하려면 다음 명령을 실행하여 수행할 수 있습니다. 스타터 플랜 또는 엔터프라이즈 플랜에서 CA의 TLS 인증서 경로에 `CORE_PEER_TLS_ROOTCERT_FILE`의 값을 설정했는지 확인하십시오.
...
체인코드가 인스턴스화되면 체인코드 조회를 사용하고 명령을 호출하여 채널 원장에서 데이터를 읽고 쓸 수 있습니다. 자세한 정보는 Hyperledger Fabric 문서에서 [피어 체인코드 ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://hyperledger-fabric.readthedocs.io/en/latest/commands/peerchaincode.html) 명령을 참조하십시오. 프록시 IP와 외부 순서 지정자 포트를 사용하여 순서 지정자 엔드포인트를 호출 명령에 전달해야 합니다. 피어 엔드포인트를 조회 명령에 전달하기만 하면 됩니다.
...
컴포넌트 로그는 명령행에서 [`kubectl CLI 명령`](#ca-kubectl-configure)을 사용하거나 ICP 클러스터에 포함된 [Kibana ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://www.elastic.co/products/kibana "Elastic Search에 대한 창")을 통하여 볼 수 있습니다.

- `kubectl logs` 명령을 사용하여 팟(Pod) 내부의 컨테이너 로그를 보십시오. 팟(Pod) 이름을 확신할 수 없으면 다음 명령을 사용하여 팟(Pod) 목록을 보십시오.
  ...
  그런 다음 `<pod_name>`을 명령 출력에서 확인한 팟(Pod) 이름으로 바꿔서 팟(Pod) 내부에 있는 피어 컨테이너의 로그를 검색하려면 다음 명령을 실행하십시오.
...
- 또는 Kibana에서 로그를 여는 [ICP 클러스터 관리 콘솔](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.1.0/troubleshoot/events.html)을 사용하여 로그에 액세스할 수 있습니다.

   **참고:** Kibana로 로그를 볼 때 응답 `No results found`를 수신할 수 있습니다. ICP에서 작업자 노드 IP 주소를 호스트 이름으로 사용하는 경우 이 상태가 발생할 수 있습니다. 이 문제점을 해결하려면 패널의 맨 위에 있는 `node.hostname.keyword`로 시작하는 필터를 제거하십시오. 그러면 로그가 표시됩니다.
...
**솔루션:**
{{site.data.keyword.cloud_notm}}(예: 스타터 또는 엔터프라이즈 플랜) 및 {{site.data.keyword.cloud_notm}} Private 둘 다에서 피어에 체인코드를 실행하려면 네트워크 모니터 UI를 사용하여 체인코드를 설치하지 마십시오. 대신 [`peer chaincode package`![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://hyperledger-fabric.readthedocs.io/en/release-1.2/commands/peerchaincode.html?highlight=peer%20chaincode%20package#peer-chaincode-package) 명령으로 체인코드를 패키지한 다음 [`peer chaincode install`](#icp-toolcontainer-install-cc) 명령을 실행하여 모든 피어에 패키지를 설치하십시오.
