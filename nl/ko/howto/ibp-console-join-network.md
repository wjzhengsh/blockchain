...
이러한 튜토리얼의 단계를 사용하여 개발 및 테스트 목적의 한 클러스터에 다중 조직이 있는 네트워크를 빌드할 수 있습니다. 블록체인 컨소시엄을 찾으려면
**네트워크 빌드** 튜토리얼을 사용하십시오. 피어를 네트워크에 연결하려면 **네트워크 가입** 튜토리얼을
사용하십시오. 다른 컨소시엄 구성원과 함께 튜토리얼에 따라 **분산** 블록체인 네트워크를 작성할 수 있습니다.  

이 튜토리얼은 피어를 **기존** 네트워크에 가입시키는 방법을 보여줍니다. 네트워크를 호스팅하는
순서 지정자가 이미 사용자의 클러스터 또는 다른 {{site.data.keyword.blockchainfull_notm}} Platform 클러스터에 존재하는 것으로
가정합니다. 가입할 기존 네트워크가 없는 경우,
[네트워크 빌드 튜토리얼](/docs/services/blockchain/howto/ibp-console-build-network.html#ibp-console-build-network)을
방문하여 작성 방법을 학습하십시오. **네트워크에 가입** 튜토리얼을 통해
...
* **하나의 채널에 가입**
  튜토리얼에서는 [네트워크 빌드 튜토리얼](/docs/services/blockchain/howto/ibp-console-build-network.html#ibp-console-build-network)에 의해 작성된
채널에 가입하는 방법을 설명합니다.
...
1. 콘솔에서 **노드** 탭을 사용하여 사용자가 작성한 `Org2 CA`로 이동하십시오.
2. 사용자의 CA를 선택한 후에 피어 자체에 대한 ID 외에 관리자를 이 조직인 `org2`에 등록해야 합니다. 이미
...
조직 관리자에 대한 등록 ID 및 시크릿, 즉, `org2admin` 및 `org2adminpw`를 제공하십시오. 그런 다음,
...
이 ID는 클러스터에 저장되거나 {{site.data.keyword.IBM_notm}}에 의해 관리되지 않습니다. 사용자의 브라우저 로컬 스토리지에만 저장됩니다.
...
6. 마지막 사이드 패널에서는 **ID 연관**을 요청하여 이를 피어의 관리자로 만듭니다. 피어 관리자 ID인 `Org2 Admin`을 선택하십시오.
...
2. `Add organizations to the channel`이라는 제목의 섹션까지 아래로 스크롤하여 `Select a channel member` 드롭 다운 목록을
열고 튜토리얼로 피어 조직 MSP인 `Org2 MSP`를 선택하십시오.
5. **추가**를 클릭한 다음 해당 조직에 대한 권한을 지정하십시오. `Operator`가 채널을 업데이트할 수 있으므로 해당 권한을 권장합니다.
6. **채널 업데이트**를 클릭하십시오.
...
3. **채널** 탭으로 이동하여 `channel1`을 클릭한 다음
**설정** 아이콘을 클릭하여 채널을 업데이트하고 피어 조직을 채널에 추가하십시오.
4. `Add organizations to the channel`이라는 제목의 섹션까지 아래로 스크롤하여 `Select a channel member` 드롭 다운 목록을
열고 튜토리얼로 피어 조직 MSP인 `Org2 MSP`를 선택하십시오.
5. **추가**를 클릭한 다음 해당 조직에 대한 권한을 지정하십시오. `Operator`가 채널을 업데이트할 수 있으므로 해당 권한을 권장합니다.
6. **채널 업데이트**를 클릭하십시오.
...
4. 후속 단계에서 ID를 연관시키도록 요청하는 경우 피어 조직 ID를 선택하십시오. 이 튜토리얼에서는 `Org2 Admin`이 되며 **제출**을 클릭하십시오.


## 3단계: 채널에 피어 가입
{: #ibp-console-join-network-join-peer-org2}

이제 거의 완료되었습니다. 피어가 기존 채널에 가입할 수 있습니다. 채널을 작성한 네트워크 운영자로부터 대역 외의 `channel name`을 가져와야 합니다. **네트워크 빌드**
튜토리얼에서 `channel1`이라는 채널을 작성했습니다. 아직 해당 위치가 아닌
...
Hyperledger Fabric [개인용 데이터 ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://hyperledger-fabric.readthedocs.io/en/latest/private-data/private-data.html "개인용 데이터") 또는
[서비스 발견 ![외부 링크 아이콘](../images/external_link.svg "외부 링크 아이콘")](https://hyperledger-fabric.readthedocs.io/en/latest/discovery-overview.html "서비스 발견") 기능을
사용하려면 **채널** 탭에서 조직 내의 앵커 피어를 구성해야 합니다. 콘솔의 **채널** 탭을 사용하여 [개인용 데이터에 대한 앵커 피어를 구성하는 방법](/docs/services/blockchain/howto/ibp-console-smart-contracts.html#ibp-console-smart-contracts-private-data)과 관련된 주제를 참조하십시오.

조직이 컨소시엄의 구성원이면 새 채널을 작성할 수도 있습니다. 단계에 따라 [네트워크 빌드 튜토리얼](/docs/services/blockchain/howto/ibp-console-build-network.html#ibp-console-build-network)에서 [채널을 작성](/docs/services/blockchain/howto/ibp-console-build-network.html#ibp-console-build-network-create-channel)하십시오.
