...
여기서 중요한 것은 오브젝트 형성을 허용하는 키 및 인증서가 적합한 CA에서 검색되는 "등록접수(enrollment)"라는 프로세스입니다. 사용자 컨텍스트 오브젝트를 형성한 후에는 이 사용자 컨텍스트를 "설정"하거나 "가져오기" 위해 애플리케이션에서 API를 호출할 수 있습니다. 이 지점에서 애플리케이션(예: 클라이언트)은 필요한 모든 아티팩트 설비를 갖추고 네트워크와 통신할 준비가 됩니다. 키와 인증서를 검색하기 위한 두 가지 접근방식을 살펴볼 것입니다.
...
두 번째로, 사용 중인 서비스 플랜에 따라 IBM Cloud에서 TLS 인증서를 다운로드하십시오.
- [스타터 플랜에 대한 루트 TLS 인증서 ![외부 링크 아이콘](images/external_link.svg "외부 링크 아이콘")](https://blockchain-certs.mybluemix.net/us2.blockchain.ibm.com.cert)
...
네트워크에서 CouchDB를 사용하는 경우 CouchDB의 성능을 향상시키는 CouchDB 색인 작성 기능을 활용하려면 색인이 체인코드와 함께 패키징되어야 합니다.

CouchDB 및 색인을 설정하는 방법에 대해 자세히 알아보려면 [CouchDB에 대한 Fabric 문서 ![외부 링크 아이콘](images/external_link.svg "외부 링크 아이콘")](http://hyperledger-fabric.readthedocs.io/en/release-1.1/couchdb_as_state_database.html)를 참조하십시오.
