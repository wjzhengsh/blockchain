...
5. Nella scheda **Dati**, aggiungi il nome utente e la password come coppie chiave/valore.
  ...
  5. Nel secondo campo **Valore**, immetti il risultato di `echo -n adminpw | base64` dal passo precedente.  
    ...
    ![{{site.data.keyword.cloud_notm}} Private - Segreto della console](../images/CreateCASecret.png "{{site.data.keyword.cloud_notm}} Private - Segreto della console")  
...
| `CA worker node architecture`| Seleziona l'architettura del tuo nodo di lavoro {{site.data.keyword.cloud_notm}} Private (ADM64 o S390X). | AMD64 | sì|
...
| `CA storage class name`| Specifica un nome della classe di archiviazione univoco. Altrimenti, viene utilizzata la classe di archiviazione predefinita nel cluster. | nessuno | Dipende dal modo in cui viene configurato il cluster {{site.data.keyword.cloud_notm}} Private. Verifica con il tuo amministratore del cluster |
