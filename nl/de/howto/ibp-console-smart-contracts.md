...
2. Klicken Sie auf **Smart Contract installieren**, um die Package-Datei mit dem Smart Contract im [.cds-Format ![External link icon](../images/external_link.svg "External link icon")](https://hyperledger-fabric.readthedocs.io/en/latest/chaincode4noah.html#packaging "packaging smart contracts") hochzuladen.
...
2. Daraufhin wird eine Seitenanzeige geöffnet. Wählen Sie einen Kanal aus, für den der Smart Contract instanziiert werden soll, und wählen Sie den Anordnungsknoten aus, in dem sich der Kanal befindet. Sie können den Kanal mit dem Namen `channel1` und den Anordnungsknoten mit dem Namen `Orderer` auswählen, die Sie erstellt haben. Klicken Sie anschließend auf **Weiter**.
