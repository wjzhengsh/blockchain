...
- Stellen Sie sicher, dass ein [{{site.data.keyword.cloud_notm}} gebührenpflichtiges Konto ![External link icon](../images/external_link.svg "External link icon")](https://console.cloud.ibm.com/catalog/services/ibm-blockchain-platform-free-20-beta "Catalog") vorhanden ist. Wenn Sie kein Konto haben:
...
Diese Ressourcen sind für Tests und Experimente Versuchsreihen ausreichend. Wenn Sie einen kostenlosen Kubernetes-Cluster verwenden, beachten Sie, dass der Cluster nach Ablauf der 30-tägigen Testversion gelöscht wird und alle zugehörigen Assets entfernt werden. Die Leistung ist in einem kostenlosen Cluster deutlich geringer.{:note}
...
1. Suchen Sie den [Blockchain-Service ![External link icon](../images/external_link.svg "External link icon")](https://console.cloud.ibm.com/catalog/services/blockchain-platform-20) im {{site.data.keyword.cloud_notm}} Katalog oder suchen Sie auf Ihrer {{site.data.keyword.cloud_notm}} Katalogseite nach `Blockchain`. 
...
1. Der Schritt **Willkommen & Voraussetzungen**. Wenn Sie bereits über einen vorhandenen {{site.data.keyword.IBM_notm}} Kubernetes Service-Cluster in der Region **Dallas** verfügen, aktivieren Sie das Kontrollkästchen. **Wenn Sie über einen IBM Kubernetes Service-Cluster verfügen und ihn für Ihren Blockchain-Service verwenden möchten, können Sie den nächsten Schritt überspringen. Stellen Sie sicher, dass die Version von Kubernetes mindestens die Version 1.11 oder höher hat.** Klicken Sie auf **Weiter**.
2. Der Schritt **Cluster erstellen**. Wenn Sie das Kontrollkästchen in Schritt 1 auswählen, um einen vorhandenen Kubernetes-Cluster zu verwenden, wird dieser Schritt übersprungen. Klicken Sie andernfalls auf **Neuen Cluster erstellen**, wodurch das {{site.data.keyword.cloud_notm}} Kubernetes-Dashboard zum Erstellen eines Clusters gestartet wird. Weitere Informationen finden Sie im Abschnitt [Einführung in {{site.data.keyword.cloud_notm}} Kubernetes Service ![External link icon](../images/external_link.svg "External link icon")](https://cloud.ibm.com/docs/containers/container_index.html). Planen Sie zusätzliche Zeit ein, um diesen Prozess abzuschließen.
  ...
  - Wählen Sie **Standard-Cluster (empfohlen):** Wenn Sie eine längerfristige Option benötigen, die mehrere Knoten für die Hochverfügbarkeit umfasst und die vom Beta-Angebot zum allgemein verfügbaren Angebot (falls schon verfügbar) migriert werden kann.
...
4. Der Schritt **In Cluster bereitstellen**. Wählen Sie den Kubernetes-Cluster aus, auf dem Sie {{site.data.keyword.blockchainfull_notm}} Platform 2.0 bereitstellen möchten, aus der Dropdown-Liste aus, und klicken Sie auf **In Cluster bereitstellen**.
  ...
  Möglicherweise treten die folgenden Situationen auf, in denen Sie in der Dropdown-Liste Ihren Kubernetes-Cluster nicht finden können:
  - Der Prozess zur Clustererstellung kann etwa 10 Minuten in Anspruch nehmen. Wenn Sie einen Cluster erstellt haben, planen Sie zusätzliche Zeit ein, bis der Status Ihres Clusters **Normal** ist.
...
Jetzt ist Ihre Konsole einsatzbereit und Sie können ein [Netz erstellen](/docs/services/blockchain/howto/ibp-console-build-network.html#ibp-console-build-network).
...
2. Wählen Sie eine Version, die höher als 1.11 ist, aus der Dropdown-Liste "Kubernetes-Version" aus und klicken Sie auf **Aktualisieren**. 
