...
{: #dev_app}
Mit Anwendungen können Sie den Chaincode aufrufen, um ein kanalspezifisches Ledger in Ihrem {{site.data.keyword.blockchain}}-Netz abzufragen oder zu aktualisieren.
{:shortdesc}
...
### Befehlszeile
Dies ist die einfachere der beiden Methoden. Befolgen Sie zuerst die Anweisungen für den Build des [Fabric CA-Clients ![Symbol für externen Link](images/external_link.svg "Symbol für externen Link")](http://hyperledger-fabric-ca.readthedocs.io/en/latest/users-guide.html). Dieser Schritt gibt Ihnen die Möglichkeit, mit einem CA-Server zu kommunizieren und von dort korrekt formatierte Zertifikate und Schlüssel zu erhalten.

Laden Sie als nächsten Schritt die TLS-Zertifikate von IBM Cloud herunter, die dem verwendeten Serviceplan entsprechen:
- [TLS-Stammzertifikat für den Starter Plan ![Symbol für externen Link](images/external_link.svg "Symbol für externen Link")](https://blockchain-certs.mybluemix.net/us2.blockchain.ibm.com.cert)
...
Öffnen Sie zum Schluss die JSON-Datei mit dem **Verbindungsprofil** auf der Anzeige **Übersicht** im Network Monitor und suchen Sie die relevanten Variablen auf:
