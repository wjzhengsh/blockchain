...
Antes de instalar iguales remotos en ICP, revise las [consideraciones sobre iguales remotos](remote_peer.html#remote-peer-limitations).
...
| `Base de datos de libro mayor`| La [base de datos de estado](../glossary.html#state-database) utilizada para almacenar el libro mayor del canal. El igual remoto tiene que utilizar la misma base de datos que la [red blockchain](../v10_dashboard.html#network-preferences). | ninguno | sí |
...
3. Para comprobar que el igual remoto se puede conectar a la red de la plataforma {{site.data.keyword.blockchainfull}}, puede ejecutar el mandato de la CLI peer desde dentro del contenedor del igual remoto. Ejecute el mandato de la CLI `peer channel fetch` para capturar el bloque de origen de un canal:
   ...
   1. Si aún no ha conectado con el clúster ICP, siga estas [instrucciones](remote_peer_operate_icp.html#remote-peer-kubectl-configure) para conectar con el mismo y utilizar la cli desde dentro del contenedor del igual.
...
Después de configurar el igual remoto en ICP, puede completar varios pasos operativos antes para enviar transacciones y leer el libro mayor distribuido de la red blockchain. Para obtener más información, consulte [Funcionamiento de un igual remoto](remote_peer_operate_icp.html).
...
Si despliega al menos dos iguales, también tiene que configurar sus [aplicaciones para alta disponibilidad](../v10_application.html#ha-app).
...
Para obtener más información sobre cómo se puede conseguir la residencia de los datos, consulte [Residencia de los datos](remote_peer.html#data-residency).
