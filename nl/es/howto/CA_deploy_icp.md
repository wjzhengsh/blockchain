...
# Despliegue de una entidad emisora de certificados en {{site.data.keyword.cloud_notm}} privado
{: #CA-deploy}
...
Asegúrese de que el clúster de ICP cumple los requisitos de recursos de hardware mínimos:
...
Si no desea utilizar los valores de almacenamiento predeterminados, asegúrese de que se configure una *nueva* `storageClass` durante la instalación de ICP; si no es así, el administrador del sistema Kubernetes tiene que crear una storageClass antes de desplegar la CA.

Puede optar por desplegar la CA en las plataformas AMD64 o S390X. No obstante, debe tener en cuenta que el
...
3. Recupere el valor de la dirección IP de proxy de clúster desde la consola de ICP. **Nota:** necesitará ser un
...
4. Cree un nombre de usuario y una contraseña de administrador de CA y almacénelos dentro de un objeto de secreto en ICP. Puede encontrar los pasos para crear el secreto en la [siguiente sección](#admin-secret).
...
5. En el separador **Datos**, añada el nombre de usuario y la contraseña como pares de clave-valor.
  ...
  5. En el segundo **Valor**, especifique el resultado de `echo -n adminpw | base64` del paso uno anterior.  
    La **Figura 1** muestra lo que puede contener el secreto de la consola ICP en función de los valores que haya especificado para las claves `ca-admin-name` y `ca-admin-password`.

    ![Secreto de la consola de ICP](../images/CreateCASecret.png "Secreto de la consola de ICP")  
    *Figura 1. Secreto de la consola de ICP*
...
**Nota:** el secreto del administrador de CA no se eliminará del clúster de ICP al suprimir el release de Helm. Usted es responsable de gestionar los secretos de su clúster de ICP. Si tiene pensado desplegar otra CA en el futuro, puede reutilizar el secreto de administrador de CA. De lo contrario, deberá suprimirlo del clúster de ICP.
...
4. Especifique los valores para los [parámetros de configuración generales](#ca-global-parameters) y acepte el acuerdo de licencia.
5. Abra el triángulo `Todos los parámetros` y especifique el valor de los [parámetros de configuración global](#ca-global-parameters).
6. Desplácese hacia abajo hasta la sección **Configuración de CA**. Marque el recuadro de selección `Instalar
CA` y rellene los [valores de configuración](#ca-parameters) de la entidad emisora de certificados.  
7. Pulse **Instalar**.
...
[cuenta de servicio ![Icono de enlace externo](../images/external_link.svg "Icono de enlace externo")](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/ "Configurar cuentas de servicio para pods") que va a utilizar para ejecutar el pod. | predeterminado | no |
...
| `Arquitectura del nodo trabajador de CA`| Seleccione su arquitectura de nodo trabajador de ICP (ADM64 o S390X). | AMD64 | sí|
...
| `Nombre de clase de almacenamiento de CA`| Especifique un nombre de clase de almacenamiento exclusivo. De lo contrario, se utilizará la clase de almacenamiento predeterminada en el clúster. | ninguno | Depende de cómo esté configurado el clúster de ICP. Consulte al administrador del clúster |
...
| `Modalidad de acceso de almacenamiento de CA`| Especifique la [modalidad de acceso](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#access-modes "modalidad de acceso") al almacenamiento para la PVC. | ReadWriteMany | sí |
...
![Icono de enlace externo](../images/external_link.svg "Icono de enlace externo")](https://www.ibm.com/support/knowledgecenter/en/SSBS6K_3.1.0/installing/install_proxy.html "Instalación de IBM Cloud privado detrás de un proxy HTTP") donde está desplegada la CA. | 127.0.0.1 | sí |
...
Tras configurar el mapa de configuración, debe reiniciar el servidor de CA para que los cambios entren en vigor. Para reiniciar el servidor de CA, puede suprimir el POD del servidor de CA de Fabric. ICP creará un nuevo POD que refleje los cambios.
