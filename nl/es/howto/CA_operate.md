...
# Funcionamiento de una entidad emisora de certificados en {{site.data.keyword.cloud_notm}} privado
{: #ca-operate}
...
- [Configure las CLI](#ca-kubectl-configure)
- [Recupere el URL de la entidad emisora de certificados](#ca-url)
- [Descargue el certificado TLS de la CA](#ca-tls)
- [Configure el cliente de CA de Fabric](#fabric-ca-client)
- [Genere certificados con el administrador de CA](#enroll-admin)
...
- [Utilización de la CA para desplegar un clasificador o un igual](#deploy-orderer-peer)
...
- [Proveedores de servicios de pertenencia (MSP)](#msp)
...
Necesitará utilizar la herramienta de línea de mandatos **kubectl** para conectarse al contenedor de CA que se ejecuta en ICP.
...
Deberá hacer referencia al URL de CA para que las solicitudes puedan generar certificados o registrarse con una nueva identidad. Puede encontrar el URL de CA utilizando la interfaz de usuario de la consola de ICP. Necesitará ser un
...
5. Si no lo ha hecho aún, siga los pasos para configurar la [CLI kubectl](#ca-kubectl-configure), que deberá utilizar para interactuar con el contenedor de CA.
...
1. Inicie una sesión en la consola de ICP. Pulse el icono **Menú** en la esquina superior izquierda.
2. Pulse **Carga de trabajo** > **Releases de Helm**.
3. Busque el nombre del release de Helm y abra el panel de detalles del release de Helm.
4. Desplácese hacia abajo hasta la sección **Notas** en la parte inferior del panel. En la sección **Notas**, puede ver un conjunto de mandatos que le ayudarán a utilizar el despliegue de la CA.
5. Si no lo ha hecho aún, siga los pasos para configurar la [CLI kubectl](#ca-kubectl-configure), que deberá utilizar para interactuar con el contenedor de CA.
...
7. El mandato también convierte el certificado al formato base64 y lo imprime. El resultado tiene un aspecto similar a la serie siguiente:
  ...
  **Importante:** debe copiar esta serie y guardarla como `certificado TLS de CA de ICP`. Lo utilizará al desplegar componentes adicionales.
...
4. Establezca el valor de la variable de entorno `$FABRIC_CA_CLIENT_HOME` en la vía de acceso donde el cliente de CA va a almacenar los certificados de [MSP](#msp) generados. Asegúrese de eliminar el material de configuración que se pueda haber creado en intentos anteriores. Si no ha ejecutado el mandato `enroll` anteriormente, la carpeta `msp` y el archivo `.yaml` no existen.
...
5. Copie el certificado TLS que ha [descargado de ICP](#ca-tls) en un directorio donde pueda hacer referencia a él en mandatos posteriores. Asegúrese de que se encuentra en el directorio `$HOME/fabric-ca-client`.
...
1. Asegúrese de completar los pasos para [configurar el cliente de CA de Fabric](#fabric-ca-client) y de que
...
[Proveedores de servicios de pertenencia](#msp).
  ...
  Si el mandato `enroll` falla, consulte el [Tema de resolución de problemas](#ca-enroll-error) para ver las causas posibles.
...
En las instrucciones siguientes se proporciona un [archivo de configuración JSON de plantilla](#config-file-template) que puede editar y guardar en el sistema de archivos local, y que le indica cómo utilizar la CA para completar este archivo.
...
En primer lugar, es necesario proporcionar la información de conexión de la CA en ICP. Utilice la información siguiente para completar los valores de la sección `"component"` del archivo:

- Los valores de `"cahost"` y `"caport"` son el URL y el puerto del
[URL de CA](#ca-url). Por ejemplo, si el URL de CA es http://9.30.94.174:30167, el valor de `"cahost"` sería `9.30.94.174` y el valor de
`"caport"` sería `30167`.
- `"caname"` es el nombre de la CA que se ha especificado al desplegar el diagrama de Helm de CA. Ha hecho referencia al nombre de CA en el mandato cuando se ha inscrito utilizando la [identidad de administrador de CA](#enroll-admin).
- `"cacert"` es el certificado TLS codificado en base64 de la CA. [Recupere el certificado TLS de la CA](#ca-tls) desde la consola de ICP utilizando la nota del release de Helm, e inserte la salida en base64 de los mandatos de la nota en la sección siguiente.
  ```
  "catls": {
    "cacert": ""
  ```
...
1. [Registre el nuevo componente con la CA](#register-component).

2. También deberá [registrar un administrador de componentes](#register-admin) y, a continuación,
[generar certificados para el administrador de componentes](#enroll-admin) dentro de una carpeta de MSP. No es necesario que complete este paso si ya ha registrado un administrador para desplegar otro componente.

3. [Registre el nuevo componente con la CA de TLS](#tls-register-component).

### Registro de la identidad de componente con la CA
{: #register-component}

Si desea encontrar un consorcio desplegando un servicio de ordenación y añadiendo organizaciones a él, o desplegar iguales y hacer que se unan a canales, en primer lugar debe registrar la identidad del componente con la CA. A continuación, el despliegue del componente puede generar los certificados necesarios para que el igual o el clasificador pueda participar en una red.

1. [Genere certificados con el administrador de CA](#enroll-ca-admin) utilizando el cliente de CA de Fabric. Utilice estos certificados de administrador para emitir los mandatos siguientes. Asegúrese de que `$FABRIC_CA_CLIENT_HOME` esté establecido en
`$HOME/fabric-ca-client/ca-admin`.
...
- Como alternativa, puede acceder a los registros y sucesos de despliegue utilizando la
