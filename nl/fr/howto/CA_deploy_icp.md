...
Vérifiez que votre cluster ICP respecte les exigences de ressources matérielle minimum :
...
- Les exigences en termes de stockage de données dépendent du nombre d'identités et de certificats stockés. Le stockage des autorités de certification n'est pas aussi important que celui d'un homologue ou d'un service de tri, mais cela dépend du cas d'utilisation. Plus le nombre d'utilisateurs est utilisé, plus l'espace est nécessaire est important. 
...
Si vous ne voulez pas utiliser les paramètres de stockage par défaut, vérifiez qu'une *nouvelle* `storageClass` est définie lors de l'installation ICP ou l'administrateur système Kubernetes doit créer une storageClass avant le déploiement de l'autorité de certification.

Vous pouvez choisir de déployer l'autorité de certification sur les plateformes AMD64 ou S390X. Toutefois, vous devez savoir que la [mise à disposition dynamique](https://kubernetes.io/docs/concepts/storage/dynamic-provisioning/) est uniquement disponible pour les noeuds AMD64 uniquement dans ICP. Par conséquent, si votre cluster combine à la fois des noeuds worker S390X et AMD64, la mise à disposition dynamique ne peut pas être utilisée.
...
5. Sous l'onglet **Données**, ajoutez le nom d'utilisateur et le mot de passe en tant que paires clé-valeur.
  ...
  5. Dans la seconde zone **Value**, entrez le résultat de `echo -n adminpw | base64` de l'étape 1 plus haut.  
    La **Figure 1** illustre ce que le secret de console ICP peut contenir selon les valeurs que vous avez indiquées pour les clés `ca-admin-name` et `ca-admin-password`.

    ![Secret de console ICP](../images/CreateCASecret.png "Secret de console ICP")  
    *Figure 1. Secret de console ICP*
...
**Remarque :** Le secret admin de l'autorité de certification n'est pas retiré de votre cluster ICP lors de la suppression de votre édition Helm. Vous êtes responsable de la gestion de vos secrets dans votre cluster ICP. Si vous prévoyez de déployer une autre autorité de certification dans le futur, vous pouvez réutiliser le secret admin de cette autorité de certification. Sinon, vous êtes responsable de sa supprimant dans votre cluster ICP.

## Configuration
{: #ca-configuration}

Une fois que vous avez créé le secret admin de votre autorité de certification, vous pouvez utilisez les étapes suivantes pour configurer et installer votre autorité de certification. Vous pouvez installer uniquement une autorité de certification à la fois.
...
4. Indiquez les valeurs relatives aux [paramètres de configuration généraux](#ca-global-parameters) et acceptez le contrat de licence.
5. Ouvrez le bouton de développement `Tous les paramètres` et indiquez la valeur des [paramètres de configuration généraux](#ca-global-parameters).
6. Faites défiler vers le bas jusqu'à la section **CA configuration**. Sélectionnez la case à cocher relative à l'`installation de l'autorité de certification` et remplissez les [paramètres de configuration](#ca-parameters) de l'homologue.  
...
| --------------|-----------------|-------|------- |
| `Helm release name`| Nom de votre édition Helm. Doit commencer par une lettre minuscule et se terminer par une caractère alphanumérique, doit contenir uniquement des traits d'union et des caractère alphanumérique minuscules. Vous devez utiliser un nom d'édition Helm unique chaque fois que vous essayez d'installer un composant. | none | yes |
...
| `Service account name`| Entrez le nom du compte de service [ ![Icône de lien externe](../images/external_link.svg "Icône de lien externe")](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/ "Configure Service Accounts for Pods") que vous allez utiliser pour lancer le pod. | default | no |
...
| `CA worker node architecture`| Sélectionnez l'architecture de votre noeud worker ICP (ADM64 ou S390X). | AMD64 | oui|
...
| `CA storage class name`| Indiquez un nom de classe de stockage unique. Sinon, la classe de stockage par défaut dans le cluster est utilisée. | aucune | Dépend de la façon dont le cluster ICP est configuré. Vérifiez auprès de votre administrateur de cluster |
...
| `CA storage access mode`| Indiquez le [mode d'accès](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#access-modes "Modes d'accès") au stockage pour le PVC. | ReadWriteMany | oui |
| `CA volume claim size`| Choisissez la taille de disque à utiliser. | 2Gi | oui |
...
| `CA CPU request`| Indiquez le nombre maximum d'UC à allouer à l'autorité de certification.| 1 | oui |
| `CA CPU limit`| Indiquez le nombre maximum d'UC à allouer à l'autorité de certification.| 2 | oui |
| `CA memory request`| Indiquez la quantité minimum de mémoire à allouer à l'autorité de certification. | 1Gi | oui |
| `CA memory limit`| Indiquez la quantité maximum de mémoire à allouer à l'autorité de certification. | 4Gi | oui |
