...
## 現行狀態 (Current state)
分類帳的現行狀態代表曾經內含在其鏈結交易日誌中所有金鑰的最新值。因為現行狀態代表頻道已知的所有最新金鑰值，所以有時稱為「廣域狀態」。鏈碼會針對現行狀態資料執行交易提案。每次金鑰值變更或新增金鑰時，現行狀態都會變更。現行狀態對於交易流程而言十分重要，因為必須知道最新金鑰值組，才能進行變更。對等節點會將最新值確定到區塊中每一個有效交易之分類帳的現行狀態。
