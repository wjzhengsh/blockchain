...
服務認證為 JSON 格式，其中包含您網路資源（亦即，CA、排序節點及對等節點）的 API 端點資訊及 enrollID/密碼。您的應用程式會透過這些 API 端點來與網路資源互動。

## 狀態資料庫
現行狀態資料儲存在對等節點的資料庫中，以從鏈碼進行有效的讀取及查詢。「入門範本方案」網路使用 CouchDB 作為狀態資料庫。「企業方案網路」可以使用 LevelDB 或 CouchDB。
...
## 排序節點 (Orderer)
收集網路成員的交易、將交易排序並將其組合成區塊的節點。這些區塊接著會分散至對等節點，而對等節點則會驗證區塊，並將其新增至每個頻道上的分類帳。排序節點包含每個成員的相關加密身分資料，並且會鑑別用戶端及對等節點的身分來存取網路。排序節點或節點集合所提供的整體功能稱為**排序服務**。

## 現行狀態 (Current state)
分類帳的現行狀態代表曾經內含在其鏈結交易日誌中所有金鑰的最新值。因為現行狀態代表頻道已知的所有最新金鑰值，所以有時稱為「廣域狀態」。鏈碼會針對現行狀態資料執行交易提案。每次金鑰值變更或新增金鑰時，現行狀態都會變更。現行狀態對於交易流程而言十分重要，因為必須知道最新金鑰值組，才能進行變更。對等節點會將最新值確定到區塊中每一個有效交易之分類帳的現行狀態。「現行」狀態儲存在對等節點的狀態資料庫中

## 組織 (Organization)
請參閱[成員](#member)。

## 聊天 (Gossip)
Hyperledger Fabric 可讓對等節點收集彼此的重要網路資訊，而不必依賴排序服務。[聊天資料散佈通訊協定 ![外部鏈結圖示](images/external_link.svg "外部鏈結圖示")](https://hyperledger-fabric.readthedocs.io/en/release-1.1/gossip.html) 提供安全、可靠且可擴充的方式，讓對等節點彼此交換訊息。例如，如果對等節點因為延遲、網路中斷或其他原因而遺失部分區塊，則它們可以使用聊天傳訊來聯絡其他對等節點（佔有這些遺漏的區塊），以同步至現行分類帳狀態。
