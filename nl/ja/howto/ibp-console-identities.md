...
アイデンティティーを CA 管理者として設定するには、**「設定」**アイコンをクリックし、スライダーの**「アイデンティティーの設定 (Set Identity)」**をクリックします。**「既存のアイデンティティー (Existing identity)」**タブを使用して、ウォレットに存在するアイデンティティーを指定できます。また、**「新規アイデンティティー (New identity)」**タブを使用して、管理者の証明書を貼り付けるか、証明書が含まれている JSON ファイルをアップロードすることができます。
...
CA で登録されたアイデンティティーごとに公開鍵と秘密鍵を生成できます。[組織の MSP を作成する](/docs/services/blockchain/howto/ibp-console-organizations.html#console-organizations-create-msp)場合、管理者アイデンティティーの 1 つを使用して鍵を生成でき、CA を使用して既にエンロールされた他の管理者の追加の証明書を MSP に提供できます。

鍵を作成するために CA で使用される[アイデンティティーが設定](/docs/services/blockchain/howto/ibp-console-identities.html#ibp-console-identities-ca-identity)されていることを確認してください。アイデンティティーを設定した後、**「アイデンティティーのエンロール (Enroll Identity)」**ボタンをクリックすると鍵を表示でき、それらはサイド・パネルに base64 形式で表示されます。
  - 公開鍵は**「証明書」**フィールドに表示されます。この証明書は、エンロールメント証明書、署名証明書、または署名付き証明書とも呼ばれます。
  - 対応する秘密鍵は**「秘密鍵」**フィールドに表示されます。

**「アイデンティティーのエンロール (Enroll Identity)」**をクリックして作成される各鍵ペアは、1 回のみ生成され、コンソールやブラウザーでは保管されません。**「アイデンティティーのエンロール (Enroll Identity)」**ボタンをクリックすると、CA 管理者で設定した最大登録数にもカウントされます。エンロールした後、アイデンティティーをローカル・ファイル・システムにダウンロードして鍵ペアを保管するか、コンソール・ウォレットに追加する必要があります。この公開鍵と秘密鍵のペアの新規名を**「名前」**フィールドに入力して、取得します。
  - **「アイデンティティーのエクスポート (Export Identity)」**をクリックして、これらの証明書をローカル・ファイル・システムに JSON 形式でダウンロードします。
...
- **証明書:** CA を使用して生成したアイデンティティーの公開鍵を貼り付けます。
- **秘密鍵:** CA を使用して生成したアイデンティティーの秘密鍵を貼り付けます。
