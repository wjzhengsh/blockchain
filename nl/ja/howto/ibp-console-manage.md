...
コンソールの許可プロセスはベータ・プログラムの期間中に変更されています。既存のブロックチェーン・サービス・インスタンスで元の {{site.data.keyword.IBM_notm}} ID 認証を使用していた場合は、**「ユーザー」**タブにユーザーが表示されなくなっています。以下に示す IAM プロセスを使用して、ユーザーにコンソールに対するアクセス権限を付与する必要があります。
{:important}

コンソールにアクセスするすべてのユーザーに、{{site.data.keyword.cloud_notm}} の IAM (ID およびアクセス管理) のユーザー役割を定義したアクセス・ポリシーを割り当てる必要があります。ユーザーがコンソール内でどのような操作を行えるかは、ポリシーによって決まります。{{site.data.keyword.blockchainfull}} Platform コンソールは、コンソール管理者である {{site.data.keyword.cloud_notm}} 所有者の E メール・アドレスを使用してプロビジョンされます。デフォルトでは、この {{site.data.keyword.cloud_notm}} ユーザーに、Blockchain Platform 2.0 サービスの`管理者`役割が IAM で付与されます。そのため、コンソール管理者は、IAM の UI を使用して、コンソールに対するアクセス権限を他のユーザーに付与できます。IAM について詳しくは、[IAM とは ![外部リンク・アイコン](../images/external_link.svg "外部リンク・アイコン")](/docs/iam?topic=iam-iamoverview#iamoverview "IAM とは") を参照してください。  
...
| 役割 |権限 |
|--------|----------|
| 管理者 | 管理者には、ライター役割を超える許可が与えられます。リーダーとライターが行えるすべての操作に加えて、以下の操作を行えます。<ul><li>新規コンポーネントを追加する。</li><li>プロビジョンされたコンポーネントを削除する。</li><li>コンソールのロギング・レベルを変更する。</li></ul> |
| ライター | ライターには、リーダー役割を超える許可が与えられます。例えば、以下の操作を行えます。<ul><li>コンポーネントをインポートする。</li><li>インポートされたコンポーネントを削除する。</li><li>ユーザーを CA に登録する。</li></ul>  |
| リーダー | リーダーは、読み取り専用の操作を行えます。例えば、以下の操作を行えます。<ul><li>コンソール UI を表示する。</li><li>コンソール・ログを表示する。</li><li>コンポーネントをエクスポートする。</li></ul> | |

 許可は累積型です。`管理者`役割を選択した場合は、そのユーザーは`ライター`と`リーダー`の操作もすべて実行できるようになるので、それらの役割にまでチェック・マークを付ける必要はありません。同様に、`ライター`役割のユーザーは、`リーダー`役割の操作をすべて実行できるようになります。コンソールにアクセスするためには、`「サービスのアクセス役割」`で役割を選択するだけでよく、`「プラットフォームのアクセス役割」`で何かを選択する必要はありません。
